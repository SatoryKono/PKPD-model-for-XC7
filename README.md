...
```bash
python -m pkpd_xc7.cli.app simulate --config examples/configs/scenarios/capsaicin.yaml --out runs/capsaicin_quickstart
```
...
В текущем пакете валидатор marker-table из legacy CLI не публикуется. Для проверки используйте тесты и загрузчики из `pkpd_xc7.io`, а для воспроизводимости ориентируйтесь на `meta.yaml`, `config_sha256` и `simulation_schema_version`.

---

## Полные примеры конфигов

Канонические scenario-конфиги находятся в `examples/configs/scenarios/`:
...
- `carrageenan.yaml`
- `hot_plate.yaml`
- `acetic_writhing.yaml`
- `zymosan.yaml`

Повторяющиеся параметры вынесены в единый `examples/configs/shared/common.yaml` и подключаются из модельного YAML через `shared_configs`.

Корневые `examples/configs/capsaicin.yaml` и `examples/configs/compound48_80.yaml` оставлены как совместимые примеры по legacy-путям, но тоже используют текущий `ModelConfig`.
