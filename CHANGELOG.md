...
- **Scenario `cyp_cystitis` (rat)**: cyclophosphamide-induced cystitis MVP with registry-defined two-phase `gaussian_sum` histamine drivers for `bladder`, `spinal_coord`, `ganglia`, and a non-proxy `brain` profile; example config `examples/configs/scenarios/cyp_cystitis.yaml` uses a 289-point `time_grid_h` (5-minute spacing on `[0, 24]` hours). Canonical tissue id `bladder` is accepted in `SHARED_TISSUE_OVERRIDE_IDS`; `examples/configs/shared/common.yaml` maps `bladder` → `peritoneum` for PK XLSX lookup when antagonist PK is enabled. Plotting adds Russian labels/colors for `bladder`; `run_all_scenarios.ps1` and `docs/plot_cyp_cystitis.py` / `docs/build_figures_docx.py` include the new figure group.
- **`cyp_cystitis` histamine YAML overrides**: optional `ModelConfig.cyp_cystitis_profile` (`FormalinProfileOverrides` shape) and per-tissue `tissue_overrides.<tissue>.formalin_profile` merge into registry gaussians via the same `_resolve_formalin_like_spec` path as formalin/intact; only `gaussian_sum` is supported for this scenario (pulse is rejected). Registry remains the default when phases are omitted.
- **XC7 competitive antagonism on G signaling**: `agonist_g_signal_fraction`, `g_signaling_fraction`, and exported `G_signal` / `G_signal_ligand` now account for tissue XC7 concentration through a pathway-specific competitive shift `H_eff = H / (1 + C / Kb_g)`. The composite G-signal formula and report-space `G_signal_pct` contract are unchanged, so `xc7_nm=0` remains backward compatible.
- **XC7 administration lag**: `antagonist_pk` now accepts `administration_lag_h`, which shifts only the XC7 PK time axis relative to model induction while keeping `time_h=0` as the scenario induction point. Positive values model delayed dosing (`xc7_nm=0` before administration), while negative values model pretreatment. The resolved lag is exported in `meta.yaml -> antagonist_pk`.
